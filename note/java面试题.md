...
* HashMap初始化在第一次put初始化，Hashtable在构造函数初始化。
* HashMap初始容量为16，HashTable初始容量为11。
* HashMap扩容2倍，Hashtable2倍+1
* HashMap没有rehash，Hashtable进行了rehash
* HashMap使用数组+链表+红黑树，Hashtable使用数组加链表
* HashMap链表采用尾插法，HashTable使用头插法。
* HashMap计算hash，Hashtable直接使用hashCode
...
### （46）TCP/IP中常用的协议
* ARP地址解析协议：在一个网络中，根据IP地址获取目标主机的MAC地址。一个路由器拿到一个发给自己直连的网络的数据报时，拿到数据报中的目的地址，然后使用ARP协议找到目的主机的MAC地址，然后交付目的主机。
* IP网际协议：IP协议制定了一套网络地址，也就是IP地址，根据IP协议能够区分两台主机是否同属一个网络（子网）。
* ICMP网际控制报文协议：允许主机或路由器报告差错情况和提供有关异常情况的报告。ICMP看起来像是高层协议，因为它的报文装在IP的数据部分。但它就是网络层的协议。
  * ICMP差错报告报文：
    * 终点不可达
    * 时间超时
    * 参数问题
    * 改变路由
  * ICMP询问报文：
    * 回送请求和回答 ：ping traceroute
    * 时间戳请求和回答

### （47）IP地址划分
网络 | 可指派的网络数 | 第一个网络号 | 最后一个网络号 | 网络中主机数 | 地址数占比
---- | ---- | ---- | ---- | ---- | ----
A | 126（2^（8-1） - 2）| 1 | 126 | 16777214（2^24 - 2）| 1/2
B | 16383（2^（16-2） - 1）| 128.1 | 191.255 | 65534（2^16 - 2）| 1/4
C | 2097151（2^（24-3） - 1）| 192.0.1 | 223.255.255 | 254（2^8 - 2）| 1/8

D 为多播地址

E 为保留地址

* 所有网络字段号为0的IP地址是个保留地址，表示本网络。
* 网络号为127保留作为本机软件环回测试本机进程的作用。
* 所有主机号全为0的IP地址表示该主机所在的网络地址。例如：5.0.0.0 是 5.6.7.8 所在的网络地址。
* 所有主机号全为1的IP地址表示本网络上的全部主机。

### 零碎知识
* MAC地址48位，6字节
* MAC帧最短64字节，首部14字节（含源、目的MAC地址）
* IP地址32位，4字节
* IP数据报首部固定部分20字节（含源、目的IP地址）
* IPv6地址128位
* 端口号16位

### 路由转发算法
* 从IP数据报中取出目的地址D，得出目的网络地址N。
* 如果N就是与该路由器直连的某一个网络，直接交付。
* 如果路由表中有目的地址D的特定主机路由，交付下一跳。
* 遍历路由表，如果有到达N的路由，交付下一跳。
* 如果有默认路由，交付默认路由。
* 报错。

### 划分子网的路由转发算法
* 从IP数据报中取出目的地址D。
* 判断是否为直接交付，遍历路由表直接相连的多个网络，用各网络的子网掩码和D做与运算，看得出的目的网络是否和该网络相同，若相同直接交付，负责执行间接交付。
* 如果路由表中有目的地址D的特定主机路由，交付下一跳。
* 遍历路由表，对每一行中的子网掩码和D做与运算，若结果匹配目的地址，交付下一跳。
* 如果有默认路由，交付默认路由。
* 报错。
...
### 三次握手
* 首先客户端A和服务端B都是closed（关闭）状态。
* B首先创建传输控制块TCB，然后进入listen（监听）状态，开始监听。
* A创建传输控制块TCB，发送连接请求报文段，SYN=1，seq=x，然后进入syn-sent（同步发送）状态。
* B收到请求后，如果同意建立连接，则向A发送确认，SYN=1，ACK=1，seq=y，ack=x+1，然后进入syn-rcvd（同步收到）状态。
* A收到确认后，发送对确认的确认，seq=x+1，ACK=1，ack=y+1，然后进入established（已建立连接）状态。
* B收到A的确认后，也进入established（已建立连接）状态。

### 四次握手
* 首先客户端A和服务端B都处于established（已建立连接状态）。
* A向B发出连接释放报文，FIN=1，seq=x，然后进入fin-wait1（终止等待1）状态。
* B收到后发出确认，ACK=1，ack=x+1，seq=y。然后进入close-wait（关闭等待）状态
* A收到确认后，进入fin-wait2（终止等待2）状态。
* B向A发出连接释放报文，FIN=1，seq=z，ACK=1，ack=x+1，然后进入last-ack（最后确认）状态。
* A收到后向B发出确认报文，ACK=1，ack=z+1，seq=x+1，进入time-wait（时间等待）状态，
* B收到后进入close（关闭）状态。
* A等待2个MSL后进入关闭状态。
